...
mostVar<-names(sort(apply(genus_abun, 1, var), decreasing = TRUE)[1:10])
leastVar<-names(sort(apply(genus_abun, 1, var))[1:10])

save(mostVar, leastVar, file = "mostVarLeastVar.RData")
